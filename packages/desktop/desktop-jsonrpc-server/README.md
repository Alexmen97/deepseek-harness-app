...
- Apiproxy surface methods (session.list/create/history/prompt/cancel, workspace.list/create, llm.providers/models) validate payloads with the same zod schemas as the HTTP carrier and invoke the same ApiProxy domain methods. Business failures ride the RpcResult error branch; JSON-RPC errors are carrier failures only.
...
Capability flags derive from the composed services (approval, userQuestions, attachments), never from version numbers. The terminal and keychain flags are fixed to false until those domains land.
...
- The terminal and keychain capability flags are fixed to false until those domains land.
