...
interface WorkflowStep { name?: string; run?: string; uses?: string; if?: string; with?: Record<string, string> }
...
function job(name: string): WorkflowJob {
  ...
  if (found === undefined) throw new Error('desktop release workflow is missing the ' + name + ' job')
  return found
}
...
describe('desktop release workflow', () => {
  ...
  it('keeps write permissions on the publish job only', () => {
    expect(job('publish').permissions).toMatchObject({ contents: 'write' })
  })

  it('creates only a draft prerelease', () => {
    const create = job('publish').steps?.find(step => step.name === 'Create draft GitHub Release')
  ...
  it('refuses tag releases without notarization credentials', () => {
    const refuse = job('build').steps?.find(step => step.name === 'Refuse unsigned tag publication')
    expect(refuse?.if ?? '').toContain('notarized')
    expect(refuse?.run ?? '').toContain('notarization credentials')
  })
  ...
  it('fails the publish gate when the SBOM asset is missing', () => {
    const sbom = job('publish').steps?.find(step => step.name === 'SBOM release gate')
