...
The shipped design is option C. The runtime composes packages/credentials/credentials-keychain, whose provider bridges resolve, store, and delete over server-initiated JSON-RPC requests on the stdio channel; the Rust desktop host answers those requests from the macOS Keychain (service com.deepseek.harness.desktop). The runtime never stores the value; the desktop host never logs it; diagnostics redact every credential-shaped value, and the frontend sees only a configured boolean with the provider name, never the raw key.
...
The Keychain service name (com.deepseek.harness.desktop) is a separate string in apps/desktop/src-tauri/src/manager.rs, not derived from the bundle identifier: changing the bundle identifier does not automatically change the service, and a signed application can keep the current service unchanged. What depends on it: the keychain provider bridge (resolve/store/delete), the saved DEEPSEEK_API_KEY entry, and this documentation. If the maintainer renames the service before the first public release, the existing development entry is orphaned in the login Keychain (recoverable through Keychain Access, deleted only by the user) and the recommended migration is a one-time re-entry of the API key; with no public users yet, an automatic migration is not warranted. Do not silently change the credential identifiers.
