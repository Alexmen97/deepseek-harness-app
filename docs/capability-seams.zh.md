...
```mermaid
flowchart LR
  pkg_desktop_jsonrpc_server["desktop-jsonrpc-server"]
  svc_desktopRuntimeInfo["ctx.desktopRuntimeInfo<br/>Desktop runtime identity provider"]
  ...
  pkg_desktop_jsonrpc_server --> svc_desktopRuntimeInfo
...
| --- | --- | --- | --- | --- | --- | --- |
| `ctx.desktopRuntimeInfo` | `core` | [`desktop-jsonrpc-server`](../packages/desktop/desktop-jsonrpc-server) | - | - | - | 为桌面 JSON-RPC 服务器提供 Harness、运行时与协议版本；组合应用二进制在树挂载前安装该值。 |
