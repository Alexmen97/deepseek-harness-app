...
| --- | --- | --- | --- | --- | --- | --- |
| `ctx.desktopRuntimeInfo` | `core` | [`desktop-jsonrpc-server`](../packages/desktop/desktop-jsonrpc-server) | - | - | - | Supplies harness, runtime, and protocol versions to the desktop JSON-RPC server; the composing app bin installs the value before the tree mounts. |
