...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/credentials/credentials-keychain': { kind: 'indirect', reason: 'The Keychain-backed provider stores credential values; consumers of ctx.credentials own any model-facing behavior.' },
  'packages/desktop/desktop-client': { kind: 'none', reason: 'Browser-side UI plugin layer; registers nothing model-facing.' },
  'packages/desktop/desktop-jsonrpc-server': { kind: 'none', reason: 'The stdio JSON-RPC carrier bridges the browser and harness gateway and registers nothing model-facing.' },
  'packages/desktop/desktop-protocol': { kind: 'none', reason: 'Wire-only type declarations; registers nothing model-facing.' },
  'packages/desktop/desktop-runtime': { kind: 'indirect', reason: 'Only the composed plugin tree contributes model context.' },
