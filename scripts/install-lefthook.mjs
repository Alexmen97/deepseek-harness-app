...
async function main() {
  ...
  // lefthook is a devDependency: absent under production-only installs (e.g. the
  // tauri CLI's dependency check). The postinstall is a no-op without it.
  let lefthookPackage
  try {
    lefthookPackage = await import('lefthook/package.json', { with: { type: 'json' } })
  } catch {
    return
  }
  const pkg = lefthookPackage.default ?? lefthookPackage
  if (typeof pkg.bin?.lefthook !== 'string') return
